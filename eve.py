...
class Eve(OptimizerDef):
  ...
  def __init__(self,
    ...
    """
    hyper_params = _EveHyperParams(learning_rate, beta1, beta2, eps,
                                   weight_decay, batch_axes)
    super().__init__(hyper_params)

  def init_param_state(self, param):
    return _EveParamState(jnp.zeros_like(param), jnp.zeros_like(param))
  ...
  def apply_param_gradient(self, step, hyper_params, param, state, grads):
    ...
    grad_sq = lax.square(grads).mean(self.hyper_params.batch_axes)
    grad_ema = beta1 * state.grad_ema + (1. - beta1) * grads.mean(self.hyper_params.batch_axes)
    ...
    new_state = _EveParamState(grad_ema, grad_sq_ema)
