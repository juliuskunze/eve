...
def train_step(state, batch, lstm_key):
  ...
  def loss_fn(params, batch, lstm_key):
  ...
  if FLAGS.use_eve:
    def elem_loss_fn(params, batch, rng):
      loss, logits = loss_fn(params, jax.tree_map(lambda x: jnp.expand_dims(x, 0), batch), rng)
      return loss, jnp.squeeze(logits, 0)
    grad_fn = jax.partial(jax.value_and_grad(elem_loss_fn, has_aux=True), state.params)
    (_, logits), grads = jax.vmap(grad_fn)(batch, jax.random.split(lstm_key, FLAGS.batch_size))
  else:
    grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
    (_, logits), grads = grad_fn(state.params, batch, lstm_key)
...
def train_model(workdir, seed):
  ...
  tx = (optax_eve.eve if FLAGS.use_eve else optax.adam)(FLAGS.learning_rate)
...
def main(_):
  with wandb.init(config=FLAGS.flag_values_dict(), sync_tensorboard=True):
    _ = train_model(FLAGS.workdir, FLAGS.seed)
