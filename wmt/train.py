...
def train_step(state,
  ...
  batch = [batch.get(k, None) for k in train_keys]
  inputs, targets, *_ = batch
  weights = jnp.where(targets > 0, 1, 0).astype(jnp.float32)
  batch = weights, *batch
  dropout_rng = jax.random.fold_in(dropout_rng, state.step)

  def loss_fn(params, dropout_rng, weights, inputs, targets, inputs_positions,
              targets_positions, inputs_segmentation, targets_segmentation):
  ...
  if state.dynamic_scale:
    if use_eve: raise NotImplementedError
    # dynamic scale takes care of averaging gradients across replicas
    grad_fn = state.dynamic_scale.value_and_grad(
        loss_fn, has_aux=True, axis_name="batch")
    dynamic_scale, is_fin, (_, logits), grads = grad_fn(state.params, dropout_rng, *batch)
    state = state.replace(dynamic_scale=dynamic_scale)
  else:
    if use_eve:
      def elem_loss_fn(params, dropout_rng, *data):
        loss, logits = loss_fn(params, dropout_rng, *jax.tree_map(lambda x: jnp.expand_dims(x, 0), data))
        return loss, jnp.squeeze(logits, 0)
      grad_fn = functools.partial(jax.value_and_grad(elem_loss_fn, has_aux=True), state.params)
      (_, logits), grads = jax.vmap(grad_fn)(jax.random.split(dropout_rng, inputs.shape[0]), *batch)
    else:
      grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
      (_, logits), grads = grad_fn(state.params, dropout_rng, *batch)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
  ...
  state = TrainState.create(
      ...
      tx=(functools.partial(optax_eve.evew, batch_axes=('batch', 0))
          if config.use_eve else optax.adamw)(
