This is a fork of the flax repository containing code for the paper "Adaptive Optimization with Examplewise Gradients".

Install via `pip install -e .`. 
Experiments are in `examples/cifar10|seq2seq|wmt|ppo|pixelcnn`. 
Experiments have additional dependencies to be installed via `pip install clu -r examples/.../requirements.txt`
The default config is the one used in the paper, with varying seeds.
See `readme`s in the respective example folders to run experiments. 
